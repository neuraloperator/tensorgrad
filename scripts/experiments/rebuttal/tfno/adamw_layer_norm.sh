...
# Base command
port=45000
base_cmd="torchrun --nproc_per_node=1 --master_port $port"

composite_common_params=" \
    --config_file ns_tensorgrad_repro_config_tfno.yaml \
    ...
    --tfno.norm group_norm \
    --tfno.group_norm_groups 1 \
    --wandb.log True \
    --tfno.n_modes [64,64] \
    --opt.tensorgrad False"

# loop over different ranks
for rank in 0.05 0.1 0.25 0.5 0.75
do
    $base_cmd train_ns_repro_tensorgrad.py $composite_common_params \
        --wandb.name LN_adamw_lr0005_tfno_r_$rank \
        --tfno.rank $rank
done
