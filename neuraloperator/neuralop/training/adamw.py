...
class AdamW(Optimizer):
    ...
    optional Tensor-Low-rank Projection (see [2]_ and [3]_) for memory-efficient training.
    ...
    def __init__(
        ...
        self.tensorgrad_scale = tensorgrad_scale
            ...
            for p in group["params"]:
                ...
                if group.get('galore', False):
                    if "projector" not in state:
                        state["projector"] = TensorGradLowRankProjector(
                            rank=self.galore_rank, 
                            update_proj_gap=self.galore_update_proj_gap,
                            scale=self.tensorgrad_scale, 
