    ...
    def forward(
        ...
        if self.complex_data:
            if self.activation_checkpoint:
                x = checkpoint(torch.fft.fftn, x, None, fft_dims, self.fft_norm, use_reentrant=False)
            else:
                x = torch.fft.fftn(x, norm=self.fft_norm, dim=fft_dims)
            dims_to_fft_shift = fft_dims
        else: 
            if self.activation_checkpoint:
                x = checkpoint(torch.fft.rfftn, x, None, fft_dims, self.fft_norm, use_reentrant=False)
            else:
        ...
        if self.activation_checkpoint:
            out_fft[slices_x] = checkpoint(self._contract, x[slices_x], weight, self.separable, use_reentrant=False)
        else:
        ...
        if self.order > 1:
            if self.activation_checkpoint:
                out_fft = checkpoint(torch.fft.fftshift, out_fft, fft_dims[:-1], use_reentrant=False)
            else:
                out_fft = torch.fft.fftshift(out_fft, dim=fft_dims[:-1])
        
        if self.complex_data:
            if self.activation_checkpoint:
                out_fft = checkpoint(torch.fft.ifftn, out_fft, mode_sizes, fft_dims, self.fft_norm, use_reentrant=False)
            else:
                out_fft = torch.fft.ifftn(out_fft, s=mode_sizes, dim=fft_dims, norm=self.fft_norm)
        else:
            if self.activation_checkpoint:
                out_fft = checkpoint(torch.fft.irfftn, out_fft, mode_sizes, fft_dims, self.fft_norm, use_reentrant=False)
            else:
