...
class FNOBlocks(nn.Module):
    ...
    def forward_with_postactivation(self, x, index=0, output_shape=None):
        # Wrap all forward calls in activation checkpointing
        if self.activation_checkpoint:
            x_skip_fno = checkpoint(self.fno_skips[index], x, use_reentrant=False)
            x_skip_fno = checkpoint(self.convs[index].transform, x_skip_fno, output_shape, use_reentrant=False)

            x_skip_channel_mlp = checkpoint(self.channel_mlp_skips[index], x, use_reentrant=False)
            x_skip_channel_mlp = checkpoint(self.convs[index].transform, x_skip_channel_mlp, output_shape, use_reentrant=False)
        ...
        if self.stabilizer == "tanh":
            if self.complex_data:
                if self.activation_checkpoint:
                    x = checkpoint(ctanh, x, use_reentrant=False)
                else:
                    x = ctanh(x)
            else:
                if self.activation_checkpoint:
                    x = checkpoint(torch.tanh, x)
                else:
                    x = torch.tanh(x)
        if self.activation_checkpoint:
            x = checkpoint(self.convs[index].forward, x, output_shape, use_reentrant=False)
        else:
        ...
        if self.norm is not None:
            if self.activation_checkpoint:
                x = checkpoint(self.norm[self.n_norms * index].forward, x, use_reentrant=False)
            else:
        ...
        if (index < (self.n_layers - 1)):
            if self.activation_checkpoint:
                x = checkpoint(self.non_linearity, x, use_reentrant=False)
            else:
                x = self.non_linearity(x)
        
        if self.activation_checkpoint:
            x = checkpoint(self.channel_mlp[index], x, use_reentrant=False)
        else:
            x = self.channel_mlp[index](x)

        x.add_(x_skip_channel_mlp)

        if self.norm is not None:
            if self.activation_checkpoint:
                x = checkpoint(self.norm[self.n_norms * index + 1], x, use_reentrant=False)
            else:
