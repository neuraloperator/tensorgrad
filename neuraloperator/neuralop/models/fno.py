...
class FNO(BaseModel, name='FNO'):
    ...
    def forward(self, x, output_shape=None, **kwargs):
        ...
        if self.activation_checkpoint:
            x = checkpoint(self.lifting, x, use_reentrant=False)
        else:
            x = self.lifting(x)
        ...
        for layer_idx in range(self.n_layers):
            if self.activation_checkpoint:
                x = checkpoint(self.fno_blocks, x, layer_idx, output_shape[layer_idx], use_reentrant=False)
            else:
        ...
        if self.activation_checkpoint:
            x = checkpoint(self.projection.forward, x, use_reentrant=False)
        else:
            x = self.projection(x)
